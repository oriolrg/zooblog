          ...
          <div class="w3-row-padding w3-section">
            ...
            @foreach($especie->seccions as $key => $seccions)
              ...
              @if(strlen($seccions->imatge)>=2)
                <div class="w3-col s3">
                  <a><img class="demo w3-opacity w3-hover-opacity-off" src="{{asset('/storage/app/public//')}}/{{$seccions->imatge}}" alt="{{$seccions->title}}" style="width:100%; height: 5em" style="" onclick="currentDiv({{$i}})"></a>
                </div>
              @endif
            @endforeach
          </div>
...
@include('public.seccions.seccions')
      ...
      <article class="text">
          @foreach($especie->seccions as $key => $seccions)
            ...
            @if(@isset($seccions->imatge))
              <article class="@if($cont %2 == 0) bg-gray @endif" id="{{$seccions->title}}">
